...
export async function GET(_req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    return NextResponse.json({ service: rowToService(result.rows[0]) });
  } catch (error) {
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const map: Record<string, string> = {
      ...
      slug: "slug", mediaType: "media_type",
    };
    ...
    const jsonMap: Record<string, string> = { features: "features", faqs: "faqs" };
